...
void FontCollectionImplTxt::RegisterFonts(
  ...
  for (const auto& family : document.GetArray()) {
    ...
    font_collection_->SetAssetFontManager(
        sk_make_sp<txt::AssetFontManager>(std::move(font_provider)));
}

void FontCollectionImplTxt::RegisterTestFonts() {
  ...
  sk_sp<SkTypeface> test_typeface =
    ...
    font_collection_->SetTestFontManager(sk_make_sp<txt::TestFontManager>(
        std::move(font_provider), GetTestFontFamilyName()));
  font_collection_->DisableFontFallback();
}

void FontCollectionImplTxt::LoadFontFromList(
